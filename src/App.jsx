...
import { SYS, ML } from "./utils/constants";
...
function App() {
  ...
  const saveRoomFlow = async (roomData) => {
    try {
      const saved = await db.saveRoom({ ...roomData, updated_at: Date.now() });
      if (saved) {
  ...
  const checkGoals = async t => {
    if (!t.trim()) return;
    try {
      const r = await db.getOrgGoals(t.trim());
  ...
  const doSaveOrgGoals = async (t, g) => {
    ...
    await db.saveOrgGoals(t.trim(), g);
  };
  ...
  const callAPI = async (sys, txt, mid, info, additionalDocs = []) => {
    ...
    try {
      ...
      const resText = await callGeminiAPI(sys, prefix + txt, docs);
      setResult(resText);
      saveLogFlow(mid, info, resText);
      if (curRoom) addHistoryFlow(curRoom, { mode: mid, modeLabel: ML[mid], result: resText, info });
    } catch (e) {
      setErr(e.message);
    } finally {
      setLoading(false);
  ...
  const handleModify = async (fb) => {
    ...
    try {
      ...
      const resText = await callGeminiAPI(SYS[1], txt, docs);
      setResult(resText);
  ...
  const gen1 = async () => {
    ...
    callAPI(SYS[1], \`[구성원]\n조직:\${f1.team} 주기:\${f1.period} 성명:\${f1.name} 레벨:\${f1.level} 직무:\${f1.role}\n기대:\${f1.expectation} Pre-Level:\${f1.preLevel}\n\n\${noOkr ? "[개인OKR] 없음" : \`[개인OKR]\n\${okr}\`}\n\n\${goalsCtx}\${existingCtx}\n\n\${noOkr ? "개인OKR 미운영. 조직목표+기대사항 기반." : "각 이니셔티브가 개인OKR의 KR 달성에 기여하는지 명시."}\`, 1, info, extraDocs);
  };

  const gen2 = () => {
    const info = { name: f2.name, role: f2.role, level: f2.level };
    callAPI(SYS[2], \`[구성원] \${f2.name} [레벨] \${f2.level} [직무] \${f2.role}\n[이니셔티브]\n\${f2.initiatives}\n[리더고민]\n\${f2.concern || "없음"}\n\n성과 1on1 아젠다 생성. 리더 고민 반영.\`, 2, info);
  };
  
  const gen3 = () => {
    callAPI(SYS[3], \`[구성원] \${f3.name}\n[미팅내용]\n\${f3.content}\n\n600자 이내 가치중심 요약.\`, 3, { name: f3.name });
  };
  
  const gen4 = () => {
    callAPI(SYS[4], \`[구성원] \${f4.name} (\${f4.role})\n[맥락] \${f4.context || "없음"}\n\n라포 1on1 아젠다 생성.\`, 4, { name: f4.name, role: f4.role });
  };

  const goalsValid = goalsMode === "pdf" ? !!goalsPdfData : goals.trim().length > 10;

  if (view === "home") {
    return <HomeView guideLoad={guideLoad} guide={guide} roomsLoad={roomsLoad} rooms={rooms} setCurRoom={setCurRoom} startMode={startMode} deleteRoom={deleteRoomFlow} setView={setView} />;
  }
  
  if (view === "room" && curRoom) {
    return <RoomView curRoom={curRoom} goHome={goHome} startMode={startMode} />;
  }
